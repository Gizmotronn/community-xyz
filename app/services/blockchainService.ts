...
export class BlockchainService {
    ...
    private emptyBlockchainData(communityId: string): CommunityBlockchainData {
        return {
    ...
    async getCommunityDataById(communityIdString: string): Promise<CommunityBlockchainData> {
        try {
            console.log(`Querying blockchain for: ${communityIdString}`);

            // Generate the community address using the same hash-based method
            const communityAddress = UserHashMappingService.generateCommunityAddress(communityIdString);
            console.log(`Generated address: ${communityAddress}`);

            // Query PointsAdded events filtered by this community address
            ...
            if (events.length === 0) {
                return this.emptyBlockchainData(communityIdString);
            }

            const processedEvents: PointsEvent[] = events.map(event => ({
                community: event.args?.community?.toString() || communityAddress,
            ...
            return {
                communityAddress: communityIdString,
                totalPoints,
            ...
            console.error(`Error fetching blockchain data for ${communityIdString}:`, error);
            return this.emptyBlockchainData(communityIdString);
        }
    }

    
    async getMultipleCommunitiesByIds(communityIds: string[]): Promise<Map<string, CommunityBlockchainData>> {
        const results = new Map<string, CommunityBlockchainData>();

        console.log(`\n Fetching blockchain rollup data for ${communityIds.length} communities...`);

        await Promise.all(
            communityIds.map(async (communityId) => {
                try {
                    const data = await this.getCommunityDataById(communityId);
                    results.set(communityId, data);
                } catch (error) {
                    console.error(`Failed to fetch data for ${communityId}:`, error);
                    results.set(communityId, this.emptyBlockchainData(communityId));
                }
            })
        );

        const communitiesWithData = Array.from(results.values()).filter(d => d.totalPoints > 0);
        const totalPoints = Array.from(results.values()).reduce((sum, d) => sum + d.totalPoints, 0);

        console.log(`Blockchain query complete:`);
        console.log(`- ${communitiesWithData.length}/${communityIds.length} communities have data`);
        console.log(`- Total blockchain points: ${totalPoints.toLocaleString()}\n`);
    ...
    async getCommunityHistoricalData(communityAddress: string): Promise<CommunityBlockchainData> {
        try {
            console.log(`[LEGACY] Fetching historical data for address: ${communityAddress}...`);
